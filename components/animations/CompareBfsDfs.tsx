...
import type { ComparisonAnimationData } from '../../lib/animations/types';
import { GraphAnimation } from './GraphAnimation';

export const CompareBfsDfs: React.FC<{ data: ComparisonAnimationData }> = ({ data }) => {
  const [step, setStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1500);

  const { bfs, dfs, description } = data;
  const maxSteps = Math.max(bfs.steps.length, dfs.steps.length);
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <h2 className="text-2xl font-bold text-center mb-4">BFS (Queue)</h2>
          <GraphAnimation 
            {...bfs}
            description=""
            externalStep={Math.min(step, bfs.steps.length - 1)}
            onStepChange={setStep}
          />
        </div>
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <h2 className="text-2xl font-bold text-center mb-4">DFS (Stack)</h2>
           <GraphAnimation 
            {...dfs}
            description=""
            externalStep={Math.min(step, dfs.steps.length - 1)}
            onStepChange={setStep}
          />
        </div>
      </div>
